...
export default function Ticket() {
    ...
    const normalize = (str) => {
      return str?.trim().toLowerCase()
        .replace(/kohlapur|kohlapur/i, 'kolhapur')
        .replace(/kohlapur/i, 'kolhapur')
        .replace(/jalagon/i, 'jalgaon')
    ...
    const done = () => {
      ...
      if (!Begi.trim() || !Desi.trim()) {
        alert('Enter the Beginning and Destination');
        return;
      }
    
      const source = normalize(Begi);
      const destination = normalize(Desi);
    
      const matchingBuses = allBus.filter(bus => {
        const from = normalize(bus.Begininig); // Keep key name as per your JSON
        const to = normalize(bus.Destination);
    ...
    return (
      <div>
        ...
        <h4 className='text-center text-uppercase text-secondary selection'>Select <br /> Beginning & Destination</h4>
  
        <div className={`main-content d-md-flex justify-content-center text-center${showpop ? ' blurred' : ''}`} id='main'>
          <label className='text-start m-3 h4 fw-semibold text-success' htmlFor="Location">Beginning</label>
          <br />
          <input
            list='Cars'
            type="text"
          ...
          <label htmlFor="Destination" className='m-3 h4 fw-semibold text-success text-end'>Destination</label>
          <br />
          <input
            list='Des'
            value={Desi}
        ...
        <div className={`main-content d-flex justify-content-center mt-5 ${showpop ? 'blurred' : ''}`}>
          <button onClick={done} className='fw-semibold text-uppercase bg-primary text-warning p-1' type="button">
            Submit
          </button>
        </div>
        ...
        {isavailable && (
          <h2 className='text-center'> No buses available for the selected route</h2>
        )}
        ...
        <div className={`main-content ${showpop ? 'blurred' : ''}`}>
          <br />
